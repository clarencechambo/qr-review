  ...
  function exportCsv() {
    ...
    const rows = filtered.map((r) =>
      [
        ...
        r.staff_rating,
        r.price_rating,
        r.discovery_channel + (r.discovery_other ? ` — ${r.discovery_other}` : ""),
        r.purchase_reason,
        r.staff_feedback ?? "",
      ]
        .map(esc)
    ...
    a.href = url;
    a.download = `dcl-reviews-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="shrink-0 rounded-full px-4 py-2 text-sm font-semibold text-white transition-colors disabled:opacity-40"
            style={{ backgroundColor: "#00ADDE" }}
            title="Download the current list as a CSV (name, phone, and details)"
          >
            Export CSV
          </button>
        </div>
